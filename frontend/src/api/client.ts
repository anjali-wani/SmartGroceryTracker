...
export async function fetchActiveInventory(householdId: number): Promise<InventoryItem[]> {
  const res = await fetch(`${API_BASE}/inventory/active?household_id=${householdId}`);
...
export async function fetchVelocityReport(householdId: number): Promise<HouseholdVelocityReport> {
  const res = await fetch(`${API_BASE}/analytics/velocity?household_id=${householdId}`);
...
export async function fetchGroceryList(householdId: number, daysAhead: number = 7): Promise<GroceryListResponse> {
  const res = await fetch(`${API_BASE}/grocery-list/generate?household_id=${householdId}&days_ahead=${daysAhead}`);
...
export async function deleteGroceryListItem(itemId: number): Promise<void> {
  const res = await fetch(`${API_BASE}/grocery-list/items/${itemId}`, {
    method: "DELETE"
  });
...
export async function toggleGroceryListItem(itemId: number, isChecked: boolean): Promise<any> {
  const res = await fetch(`${API_BASE}/grocery-list/items/${itemId}/toggle?is_checked=${isChecked}`, {
    method: "POST"
  });
...
export async function addManualGroceryItem(
  ...
  const res = await fetch(`${API_BASE}/grocery-list/items?household_id=${householdId}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function exportGroceryListText(householdId: number): Promise<string> {
  const res = await fetch(`${API_BASE}/grocery-list/export?household_id=${householdId}&format=text`);
  if (!res.ok) throw new Error("Failed to export list");
  return res.text();
}

export async function uploadReceiptCsv(file: File, householdId: number): Promise<CSVUploadSummary> {
  const formData = new FormData();
  formData.append("file", file);
  const res = await fetch(`${API_BASE}/bills/upload-csv?household_id=${householdId}`, {
    method: "POST",
    body: formData
  });
  if (!res.ok) {
...
export async function resetDatabase(householdId?: number, reseed: boolean = true): Promise<any> {
  const url = householdId !== undefined
    ? `${API_BASE}/system/reset?household_id=${householdId}&reseed=${reseed}`
    : `${API_BASE}/system/reset?reseed=${reseed}`;
...
export async function addAliasToItem(itemId: number, rawAlias: string): Promise<any> {
  const res = await fetch(`${API_BASE}/items/${itemId}/aliases?raw_alias=${encodeURIComponent(rawAlias)}`, {
    method: "POST"
  });
  if (!res.ok) {
...
export async function fetchUploadedBills(householdId: number): Promise<ReceiptUpload[]> {
  const res = await fetch(`${API_BASE}/bills?household_id=${householdId}`);
...
export async function fetchBillItems(billId: number): Promise<BillItem[]> {
  const res = await fetch(`${API_BASE}/bills/${billId}/items`);
...
export async function deletePurchaseItem(purchaseId: number): Promise<any> {
  const res = await fetch(`${API_BASE}/bills/purchases/${purchaseId}`, {
    method: "DELETE"
  });
  if (!res.ok) {
...
export async function fetchItemDetails(itemId: number, householdId: number = 1): Promise<ItemDetails> {
  const res = await fetch(`${API_BASE}/items/${itemId}/details?household_id=${householdId}`);
  if (!res.ok) throw new Error("Failed to fetch item details");
  return res.json();
}

export async function deleteItemAlias(aliasId: number): Promise<void> {
  const res = await fetch(`${API_BASE}/items/aliases/${aliasId}`, {
    method: "DELETE"
  });
...
export async function updateItemAlias(aliasId: number, rawAlias: string): Promise<ItemAlias> {
  const res = await fetch(`${API_BASE}/items/aliases/${aliasId}?raw_alias=${encodeURIComponent(rawAlias)}`, {
    method: "PATCH"
  });
...
export async function updateItemPrice(itemId: number, price: number): Promise<CanonicalItem> {
  const res = await fetch(`${API_BASE}/items/${itemId}/schedule`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
